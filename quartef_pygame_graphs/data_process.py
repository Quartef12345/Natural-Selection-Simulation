...
def raw_data(self, data):
    ...
    for y_data in y_data_dic:
        for point in y_data_dic[y_data]:
...
def compress_data(data, original_data, density, compression_method):   #Density = Maximum ammount of visible points
    ...
    if compression_method == "index_jump":  
        ...
        x_array = original_data["x"][1]
        compressed_x_data = []
        total_points = len(x_array)
        jump_distance = int(total_points/density)   #The ammount you jump over
        for metric, values in original_data["y_dic"].items():     # Repeats the compression method for each metric
            ...
            if total_points <= density:         #If there are less points than density, skips the compression at all
                ...
                for n in range(0, total_points, jump_distance):        #Goes trough each point valid point in the metric
                        compressed_values.append(values[n]) #Adds them to the sub final result
                if(values[-1] != compressed_values[-1]):
                    compressed_values.append(values[-1])


            compressed_y_data[metric] = compressed_values    #Adds the sub final result to the final result


        compressed_x_data = []
        if total_points <= density:         #If there are less points than density, skips the compression at all
            compressed_x_data = original_data["x"][1]
        else:
        ...
        for metric, values in data["y_dic"].items():     # Repeats the compression method for each metric
            compressed_values = []  #The sub final result
            if total_points <= density:         #If there are less points than density
                compressed_values = values
                compressed_x_data = x_array
            else:
                compressed_values.append(values[0])
                compressed_x_data.append(x_array[0])
                last_point = x_array[0]
                for n in range(total_points):
                    if x_array[n] - last_point >= jump_distance:
                        compressed_values.append(values[n])
                        compressed_x_data.append(x_array[n])
                        last_point = x_array[n]
                if compressed_x_data[-1] != x_array[-1]:
                    compressed_values.append(values[-1])
                    compressed_x_data.append(x_array[-1])
            compressed_y_data[metric] = compressed_values   #Adds the sub final result to the final result
